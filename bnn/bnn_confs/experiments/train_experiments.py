...
parser.add_argument("--calc_curvatures", type=str, default="True")
    ...
    for lr in lrs:
        for _ in range(trials):
            if other_args is not None:
                os.system(
                    f'python monge_bnn/experiments/train_bnn.py with data={data} model={model} weight_prior={prior}{bias_prior}inference={inference} lr={lr} weight_scale=1.41 sampling_decay={sampling_decay} batch_size={batch_size} width={width} temperature={temperature} save_samples={save_samples} progressbar=False log_dir=monge_bnn/results/{parent_dir}/{inference}_{lr}_{"_".join(other_args.split())} batchnorm=True uncertainty_plot=False cycles={cycles} epochs_per_cycle={epochs_per_cycle} burnin_batches={burnin_batches} calc_curvatures={calc_curvatures} {other_args}'
                )
            else:
                os.system(
                    f"python monge_bnn/experiments/train_bnn.py with data={data} model={model} weight_prior={prior}{bias_prior}inference={inference} lr={lr} weight_scale=1.41 sampling_decay={sampling_decay} batch_size={batch_size} width={width} temperature={temperature} save_samples={save_samples} progressbar=False log_dir=monge_bnn/results/{parent_dir}/{inference}_{lr} batchnorm=True uncertainty_plot=False cycles={cycles} epochs_per_cycle={epochs_per_cycle} burnin_batches={burnin_batches} calc_curvatures={calc_curvatures}"
                )
...
run_exp(
    ...
    calc_curvatures=args.calc_curvatures,
)
