...
def main(
    ...
    with exp_utils.HDF5Metrics(
        ...
        if calc_curvatures:
            evaluations, curvatures = mcmc.run(progressbar=progressbar)
        else:
            evaluations = mcmc.run(progressbar=progressbar)
    model.eval()
    ...
    final_evaluation = {
        "val": dict(list(evaluations["val"][-1].items())[:5]),
        "test": dict(list(evaluations["test"][-1].items())[:5]),
    }
