# Taken from https://github.com/google/uncertainty-metrics
#
# coding=utf-8
...
__all__ = ("ece", "ace", "tace", "sce", "rmsce")
...
def one_hot_encode(labels, num_classes=None):
    ...
    return np.eye(num_classes)[np.squeeze(labels)]
...
class GeneralCalibrationError:
    ...
    def get_calibration_error(self, probs, labels, bin_upper_bounds, norm, num_bins):
        ...
        self.accuracies = (
            np.bincount(bin_indices, weights=labels, minlength=num_bins) / counts
        )
    ...
    def update_state(self, labels, probs):
        ...
        if self.binning_scheme == "even":
            bin_upper_bounds = np.histogram_bin_edges(
                [], bins=self.num_bins, range=(0.0, 1.0)
            )[1:]
