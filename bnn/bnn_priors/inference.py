...
class WenzelSGLDRunner:
    ...
    def run(self, progressbar=False):
        ...
        if self.calc_curvatures:
            return evaluations, timestamps, curvatures
        else:
            return evaluations, timestamps
...
class SASGLDRunner(WenzelSGLDRunner):
    def __init__(
        self,
        model,
        dataloader,
        ...
        # SASGLD hyperparameters
        self.sas_alpha = sas_alpha
        self.sas_m = sas_m
        self.sas_M = sas_M
        self.sas_r = sas_r
        self.sas_s = sas_s
        self.sas_init_z = sas_init_z
    ...
    def _make_optimizer(self, params):
        return mcmc.SASGLD(
            ...
            num_data=self.eff_num_data,
            temperature=self.temperature,
            alpha=self.sas_alpha,
            m=self.sas_m,
            M=self.sas_M,
            r=self.sas_r,
            s=self.sas_s,
            init_z=self.sas_init_z,
        )
