...
class AbstractModel(nn.Module, abc.ABC):
    ...
    def _log_likelihood(self, x: torch.Tensor, y: torch.Tensor):
        # compute batch size using zeroth dim of inputs (log_prob_batch doesn't work with RaoB).
        batch_size = x.shape[0]
        preds = self(x)
        return preds.log_prob(y).sum() / batch_size
    ...
    def potential_avg(self, x, y, eff_num_data):
        "-log p(y, params | x)"
        return -self._log_likelihood(x, y) - self.log_prior() / eff_num_data
