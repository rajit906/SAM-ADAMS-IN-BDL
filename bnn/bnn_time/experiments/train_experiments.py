    ...
    for lr in lrs:
        for _ in range(trials):
            if other_args is not None:
                os.system(
                    f'python bnn_time/experiments/train_bnn.py with data={data} model={model} weight_prior={prior}{bias_prior}inference={inference} lr={lr} weight_scale=1.41 sampling_decay={sampling_decay} batch_size={batch_size} width={width} temperature={temperature} save_samples={save_samples} progressbar=False log_dir=bnn_time/results/{parent_dir}/{inference}_{lr}_{"_".join(other_args.split())} batchnorm=True uncertainty_plot=False cycles={cycles} epochs_per_cycle={epochs_per_cycle} burnin_batches={burnin_batches} {other_args}'
                )
            else:
                os.system(
                    f"python bnn_time/experiments/train_bnn.py with data={data} model={model} weight_prior={prior}{bias_prior}inference={inference} lr={lr} weight_scale=1.41 sampling_decay={sampling_decay} batch_size={batch_size} width={width} temperature={temperature} save_samples={save_samples} progressbar=False log_dir=bnn_time/results/{parent_dir}/{inference}_{lr} batchnorm=True uncertainty_plot=False cycles={cycles} epochs_per_cycle={epochs_per_cycle} burnin_batches={burnin_batches}"
                )
